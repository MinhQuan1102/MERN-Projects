...
const Hotel = () => {
  ...
  const { data, loading, error } = useFetch(`http://localhost:5000/api/hotels/find/${hotelId}`)
  const { state } = useSearchContext()
  const { user } = useAuthContext()
  const {options, dates} = state
  ...
  let totalPrice 
  if (data.success) { 
    totalPrice = days * hotel.cheapestPrice * options.room
  }
